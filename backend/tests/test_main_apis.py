...
async def test_create_learning_progress(client: AsyncClient, test_user):
    ...
    response = await client.post("/study/progress", json={"word_id": word_id}, headers=headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_create_duplicate_progress(client: AsyncClient, test_user):
    ...
    # 第一次创建
    response1 = await client.post("/study/progress", json={"word_id": word_id}, headers=headers)
    assert response1.status_code == 200

    # 第二次创建应该返回已存在的记录
    response2 = await client.post("/study/progress", json={"word_id": word_id}, headers=headers)
...
async def test_word_review(client: AsyncClient, test_user):
    ...
    response = await client.post("/study/review", json=review_data, headers=headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_get_needs_review(client: AsyncClient, test_user):
    ...
    response = await client.get("/study/needs-review", headers=headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_chat_endpoint_without_mock(client: AsyncClient, test_user):
    ...
    try:
        response = await client.post("/chat/", json=chat_data, headers=headers, timeout=30.0)
...
async def test_clear_chat_history(client: AsyncClient, test_user):
    ...
    response = await client.post("/chat/clear", headers=headers)
    assert response.status_code == 200
...
async def test_unauthorized_access(client: AsyncClient):
    """测试未授权访问"""
    # 尝试访问需要认证的端点
    response = await client.get("/study/needs-review")
    assert response.status_code == 401

    response = await client.post("/study/progress", json={"word_id": 1})
    assert response.status_code == 401
