...
async def test_full_learning_flow(client, test_user):
    ...
    # 1. 创建学习进度
    resp = await client.post("/study/progress", json={"word_id": word_id}, headers=headers)
    ...
    # 2. 第一次复习（5分）
    resp = await client.post("/study/review", json={"word_id": word_id, "quality": 5}, headers=headers)
    data = resp.json()
    ...
    # 3. 第二次复习（4分）
    resp = await client.post("/study/review", json={"word_id": word_id, "quality": 4}, headers=headers)
    data = resp.json()
    assert data["repetitions"] == 2
    assert data["interval"] == 6

    # 4. 第三次复习（0分，忘记）
    resp = await client.post("/study/review", json={"word_id": word_id, "quality": 0}, headers=headers)
    data = resp.json()
...
async def test_sm2_algorithm_cases(client, test_user):
    ...
    for i, quality in enumerate(first_review_cases):
        ...
        if word_id <= test_user["word_id"] + 10:  # 确保不超出测试数据范围
            ...
            # 进行复习
            resp = await client.post("/study/review", json={"word_id": word_id, "quality": quality}, headers=headers)
            data = resp.json()
